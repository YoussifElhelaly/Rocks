...
$(document).ready(function() {
    if ('IntersectionObserver' in window) {
        ...
        // Initial check in case some images are in the viewport on load
        lazyLoadImages();
    }
});
// ----------- side menu -----------
...
document.addEventListener('DOMContentLoaded', () => {
    const digits = [
        ...
        document.getElementById('digit-10'),
        document.getElementById('digit-11'),
        document.getElementById('digit-12'),
        document.getElementById('digit-13'),
        document.getElementById('digit-14'),
        document.getElementById('digit-15'),
    ];

    const updateDigit = (digitElement, delay) => {
        setTimeout(() => {
            digitElement.style.animation = 'none';
            requestAnimationFrame(() => {
                setTimeout(() => {
                    digitElement.style.animation = 'flip 0.8s ease forwards';
                    digitElement.textContent = '0';
                }, 50); // Delay to allow the animation to reset
            });
        }, delay);
    };
    ...
    const flipCounter = () => {
        digits.forEach((digit, index) => {
            updateDigit(digit, index * 200); // Adjust the timing as needed
        });
    };
